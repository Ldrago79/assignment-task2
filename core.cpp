...
ENGINE_API bool
ShouldCall PreReplicationForReplay() const;
...
/** If true, this actoris o longer replicated to new clients, and is "torn off" (becomes a ROLE_Authority) on clients to which it was being replicated. */
...
#if WITH_EDITOR
/** Deprecated for a nonvirtual version.**/
UE_DEPRECATED(5.4, "Xall IslnnEditLevelInstanceHierarchy/IsInEditLevelInstance instead.")
virtual bool IsInEditingLevelInstance() const
{
return blslnEditLevelInstance;
}

/** If true, the cator belongs to a level instance which is currently being edited*/
...
*Causes all initalization to be performed again even though the actor may not have actaually been reloaded.
*/
UPROPERTY(Transient)
uint8 bExchangedRoles:1;

/** Thisn actor will be loaded on network clients during map load*/
UPROPERTY(Category= REplication, EditAnywhere)
uint8 bNetLoadOnClient:1;

/** If actor has valid owner, call owner's IsNetReleventFor and GetNetPriority*/
UPROPERTY(Category = Replication, EditDeafaultsOnly, BlueprintReadWrite)
uint8 bNetUseOwnerRelevancy:1;

/** If true, this actor will be replicated tto network replays(deafult is true)*/
UPROPERTY()
uint8 bRelevantForNetworkReplays:1;

/**
*If true, this actor's component's bounds will be included in thhe level;s
*bounding box unless the Actors's class has overridden IsLevelBoundsRelevant */
URPROPERTY(EditAnywhere, Category = Collision, AdvancedDisplay)
...
URPROPERTY(Category = Replication, EditDefaultsOnly, AdvancedDisplay)
uint8 bReplayRewindable:1;

/** 
WHether we allow this actor to tick before it receieves the BeginPly Event.
*Normally we don't tick actors until after BeginPlay; this setting allows this behaviour to be overridden.
* This Actor must be ableto tick for this setting to be relevant.
*/
...
UPROPERTY(BlueprintSetter = SetAutoDestroyWhenFinished:1;

/**
* Whether this actor can take adamange. Must be true for damage events(e.g. RecieveDamage()) bto be called. */
...
public:
/**If true, all input on the stackbelow this actor will not be considered*/
UPROPERTY(EditDeafaultsOnly,Category = Input)
uint8 bBloackInput:1;

/**
This actor collides with the world when placing in the editor, even if RootComponent collision is disabled. Does not affect spwning. */
UPROPERTY()
UINT8 BCollideWhenPlacing:1;

/**If true, this actor should search for an owned camera component to view through when used as a view targett. */
UPROPERTY(EditAnywher, BlueprintReadWrite, Category = Actor, AdvancedDisplay)
uint8 bFindCameraComponentWhenViewTarget1;

/**If true, this actoor will generate overlap Begin/End ebvents when spawned as part of level streaming, which included initial level load.
*You might enable this in the case where a streaming level loads around an actor and you want Begin/End overlap events to trigger. */
UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Collision)
uint8 bGenerateOverlapEventsDuringLevelStreaming:1;

/** Whether this actor should not be affected byb b world origin shifting. */
...
UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = HLOD, meta = (DisplayNmae = "Include Actor in HLOD"))
uint8 bEnableAutoLODGeneration:1;

/** Wether this actor is editor-only. Use with care, as if this actor is refernced by anything else that reference will be NULL in cooked builds */
...
UPROPERTY()
uint8 bActorSeamlessTraveld:1;

/**
* Does this actor have an owner responible fpr replication?( APlaterController typically) */
ENGINE_API virtual bool HasNetOwner() const;

/** Does this actor have a logically controlled owner responsibe for replication? 
...
bool GetAutoDestroyWhenFinished() const { return bAutoDestroyWhen Finished;}

UFUNCTION(BlueprintSetter)
ENGINE_API void SetAutoDestroyWhen Finished(bool bVal);
...
AUnrealCPPProjectile::AUnrealCPPProjectile() 
{
	...
	CollisionComp->OnComponentHit.AddDynamic(this, &AUnrealCPPProjectile::OnHit);		// set up a notification for when this component hits something blocking
...
void AUnrealCPPProjectile::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
